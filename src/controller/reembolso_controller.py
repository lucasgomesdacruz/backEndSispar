...
from datetime import datetime
from flasgger import swag_from
from src.security.jwt_manager import token_required
...
@token_required
@swag_from('../docs/reembolso/create_reembolso.yml')
def criar_reembolso(current_user):
    if request.method == "OPTIONS":
        return '', 200
    ...
    try:
        novo_reembolso = Reembolso(
            ...
            valor_faturado=data['valor_faturado'],
            despesa=data.get('despesa'),
            id_colaborador=current_user,  # Usa o ID do usuário do token
            status=data.get('status', 'Em analise')
        )
        db.session.add(novo_reembolso)
        db.session.commit()
...
@bp_reembolso.route('/reembolsos/<int:num_prestacao>', methods=['GET'])
@token_required
def visualizar_reembolso(current_user, num_prestacao):
    try:
        reembolso = Reembolso.query.filter_by(num_prestacao=num_prestacao, id_colaborador=current_user).first()
        if not reembolso:
            return jsonify({'error': 'Reembolso não encontrado'}), 404
...
# Listar todos os reembolsos (apenas para administradores)
@bp_reembolso.route('/pegar-todos-reembolsos', methods=['GET'])
@token_required
def listar_todos_reembolsos(current_user):
    # Verifique se o usuário é admin (você pode adicionar essa lógica)
...
@bp_reembolso.route('/reembolsos', methods=['GET'])
@token_required
def listar_reembolsos(current_user):
    reembolsos = Reembolso.query.filter_by(id_colaborador=current_user).all()
...
@bp_reembolso.route('/reembolsos/<int:id>', methods=['GET'])
@token_required
def obter_reembolso(current_user, id):
    reembolso = Reembolso.query.filter_by(id=id, id_colaborador=current_user).first()
    if not reembolso:
        return jsonify({'message': 'Reembolso não encontrado'}), 404
...
@bp_reembolso.route('/reembolsos/<int:id>', methods=['PUT'])
@token_required
def atualizar_status(current_user, id):
    reembolso = Reembolso.query.filter_by(id=id, id_colaborador=current_user).first()
    if not reembolso:
        return jsonify({'message': 'Reembolso não encontrado'}), 404
...
@bp_reembolso.route('/reembolsos', methods=['DELETE'])
@token_required
def deletar_reembolso_do_colaborador(current_user):
    dados = request.get_json()
    id_reembolso = dados.get('id')
    ...
    reembolso = Reembolso.query.filter_by(id=id_reembolso, id_colaborador=current_user).first()
...
@bp_reembolso.route('/reembolsos/resumo', methods=['GET'])
@token_required
def resumo_reembolsos(current_user):
    try:
        total_solicitados = db.session.query(Reembolso).filter_by(id_colaborador=current_user).count()

        aprovados = db.session.query(Reembolso).filter_by(id_colaborador=current_user, status='Aprovado').count()
        rejeitados = db.session.query(Reembolso).filter_by(id_colaborador=current_user, status='Rejeitado').count()

        em_analise = db.session.query(Reembolso).filter(
            Reembolso.id_colaborador == current_user,
            ~Reembolso.status.in_(['Aprovado', 'Rejeitado'])
        ).count()
...
@bp_reembolso.route('/reembolsos/resumo/unico', methods=['GET'])
@token_required
def resumo_unico_reembolsos(current_user):
    try:
        total_solicitados = db.session.query(Reembolso).filter_by(id_colaborador=current_user).count()
